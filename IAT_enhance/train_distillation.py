...
class LightweightGammaPredictor(nn.Module):
    ...
    def __init__(self, in_channels=3):
        ...
        self.downsample = nn.AdaptiveAvgPool2d((64, 64))  # 고정 크기로

        # ✅ 변경: gamma_net과 color_net을 따로 만들지 말고 공유
...
class IAT_Student(nn.Module):
    ...
    def apply_color(self, image, ccm):
        ...
        """
        b, c, h, w = image.shape
        image = image.view(b, c, -1)  # [B, 3, H*W]
        image = torch.bmm(ccm, image)  # [B, 3, 3] x [B, 3, H*W] = [B, 3, H*W]
    ...
    def forward(self, img_low, return_intermediates=False):
        ...
        b = img_local.shape[0]
        img_high = img_local  # 이미 [B, C, H, W]

        # Color correction (배치 전체)
        img_high = self.apply_color(img_high, color)

        # Gamma correction (배치 전체)
        img_high = img_high ** gamma.view(b, 1, 1, 1)
...
class IAT_DistillationTrainer:
    def __init__(self, config):
        ...
        if config.resume_path is not None and os.path.exists(config.resume_path):
            ...
            # ✅ optimizer, scheduler는 새로 초기화 (다시 학습 시작)
            self.start_epoch = config.resume_epoch
            print(f"▶️ Resumed model weights. Continue training from epoch {self.start_epoch}")
    ...
    def train(self, train_loader, val_loader):
        best_psnr = 0 if not hasattr(self, "best_psnr") else self.best_psnr
